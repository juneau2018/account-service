...
public class AccountControllerTest {
	...
	public void shouldGetAccountByName() throws Exception {
		...
		mockMvc.perform(get("/" + account.getName())).andExpect(jsonPath("$.name").value(account.getName()))
	...
	public void shouldGetCurrentAccount() throws Exception {
		...
		mockMvc.perform(get("/current").principal(new UserPrincipal(account.getName())))
				.andExpect(jsonPath("$.name").value(account.getName())).andExpect(status().isOk());
	}
	...
	public void shouldRegisterNewAccount() throws Exception {
		...
		mockMvc.perform(
				post("/").principal(new UserPrincipal("test")).contentType(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isOk());
	}
